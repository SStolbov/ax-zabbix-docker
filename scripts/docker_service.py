...
def discover():
	...
	pipe2 =  os.popen("docker node inspect self --format '{{ .Spec.Role }}'")
	for line in pipe2:
			if line.strip() == "manager":
				with os.popen("docker service ls --format \"{{.Name}}\"") as pipe:
					for line in pipe:
					    ps = {}
#					    print(line.strip())
					    ps["{#SERVICENAME}"] = line.strip()
					    d["data"].append(ps)
			print (json.dumps(d))
#			ps = {}
#			reg = r'^.+?\.[\d]+'
